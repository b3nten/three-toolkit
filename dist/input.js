...
export class InputQueue {
  ...
  onInputDown(key, callback) {
    const keys = Array.isArray(key) ? key : [key];
    const cleanup = [];
    for (const k of keys) {
      if (!this.inputDownCallbacks.has(k)) {
        this.inputDownCallbacks.set(k, /* @__PURE__ */ new Set());
      }
  ...
  onInputUp(key, callback) {
    const keys = Array.isArray(key) ? key : [key];
    const cleanup = [];
    for (const k of keys) {
      if (!this.inputUpCallbacks.has(k)) {
        this.inputUpCallbacks.set(k, /* @__PURE__ */ new Set());
      }
  ...
  states = {};
  inputDownCallbacks = /* @__PURE__ */ new Map();
  inputUpCallbacks = /* @__PURE__ */ new Map();
  keydown(e) {
    if (!this.enabled) return;
    const key = e instanceof KeyboardEvent ? e.key : e instanceof WheelEvent ? `wheel${e.deltaY > 0 ? "Down" : "Up"}` : `mouse${e.button}`;
    this.queue.push({ key, state: { pressed: true } });
  }
  keyup(e) {
    if (!this.enabled) return;
    const key = e instanceof KeyboardEvent ? e.key : e instanceof WheelEvent ? `wheel${e.button}` : `mouse${e.button}`;
